...
class Arguments:
    def __init__(self, input_file, output_directory, segmented):
        self.i = input_file
        self.o = output_directory

        self.k = [1, 0, 0]
        self.d = [3.26, 3.26, 3.0] # spacing in um
	self.truecells = segmented
	self.Iavg_min = 0  # minimum allowed average intensity from main.raw
	self.azmin = 0    # index of first atlas slice in z
        self.azmax = 250  # index of last atlas slice in z
        self.ori = [1,2,3]   # order and orientation of axis 1,2,3

        self.affine = "data/affine.txt"  # path to affine_parameter_file
        self.bspline = "data/bspline.txt"  # path to bspline_parameter_file



def process(input_file, output_directory, segmented):

    me = "registration.py"

    args = Arguments(input_file, output_directory, segmented)
